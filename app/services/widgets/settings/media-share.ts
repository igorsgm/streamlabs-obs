...
export class MediaShareService extends WidgetSettingsService<IMediaShareData> {
  getApiSettings() {
    return {
      ...
      previewUrl: `http://${this.getHost()}/widgets/media-share?token=${this.getWidgetToken()}`,
      settingsUpdateEvent: 'mediaSharingSettingsUpdate',
      goalCreateEvent: 'newmediaShare',
      goalResetEvent: 'mediaShareEnd',
      dataFetchUrl: `http://${this.getHost()}/api/v5/slobs/widget/media`,
      settingsSaveUrl: `http://${this.getHost()}/api/v5/slobs/widget/media`,
